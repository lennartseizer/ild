...
**Nakagawa’s R2**  
R<sup>2</sup><sub>m</sub> = ( σ<sup>2</sup><sub>fixed</sub> ) / (
σ<sup>2</sup><sub>fixed</sub> + σ<sup>2</sup><sub>random</sub> +
σ<sup>2</sup><sub>residual</sub> )       
...
**Example RQ:**  
within-participant `eventpl_w` -\> `pa` with covariates `eventpl_bc`,
`use_alcohol` and `use_coffee`
...
```
    ...
    ##  Structure: General positive-definite, Log-Cholesky parametrization
    ##             StdDev     Corr  
    ## (Intercept) 0.83384088 (Intr)
    ## eventpl_w   0.09158526 -0.143
    ## Residual    0.94343314       
    ## 
    ## Fixed effects:  pa ~ eventpl_w * status 
    ##                               Value  Std.Error    DF  t-value p-value
    ## (Intercept)                5.188217 0.07999712 13708 64.85505  0.0000
    ## eventpl_w                  0.137072 0.01311976 13708 10.44774  0.0000
    ## statusdepressed           -1.111828 0.11385890   325 -9.76497  0.0000
    ## statuspsychotic           -0.813189 0.11470354   325 -7.08948  0.0000
    ## eventpl_w:statusdepressed  0.125438 0.01791223 13708  7.00291  0.0000
    ## eventpl_w:statuspsychotic  0.021742 0.01927807 13708  1.12784  0.2594
    ##  Correlation: 
    ...
    ## Standardized Within-Group Residuals:
    ##         Min          Q1         Med          Q3         Max 
    ## -5.79477493 -0.50506742  0.09082458  0.58929774  4.28090251 
    ## 
    ## Number of Observations: 14039
    ## Number of Groups: 328
...
data_med$dv <- factor(data_med$dv, levels = c("m","pa"))  
...
```
    ...
    ##         id   obs use_alcohol eventpl_w dv         z dm dy
    ## 1     c100 -29.5           0      -2.4  m -2.400000  1  0
    ## 19681 c100 -29.5           0      -2.4 pa  4.666667  0  1
    ## 2     c100 -28.5          NA        NA  m        NA  1  0
    ## 19682 c100 -28.5          NA        NA pa        NA  0  1
    ## 3     c100 -27.5           0      -0.4  m -0.400000  1  0
    ## 19683 c100 -27.5           0      -0.4 pa  5.333333  0  1
...
`dm:use_alcohol` - effect of `x` when `m` is the outcome (`a` path)  
`dy:eventpl_w` - effect of `m` when `y` is the outcome (`b` path)  
`dy:use_alcohol` - effect of `x` when `y` is the outcome (`c` path)
...
Detailed tutorial:  
<https://solomonkurz.netlify.app/blog/2021-01-26-effect-sizes-for-experimental-trials-analyzed-with-multilevel-growth-models-one-of-two/>
...
# d_gma
as.numeric(fixef(m13)["day:statuspsychotic"]) * 5 / pooled_sd
```
...
> 1.(a) Estimate a model that predicts negative affect (`na`) based on
> whether participants are alone or with others (`soc_alone`). (b) Test
> whether the previous effect is moderated by how pleasant the company
> of others is experienced `soc_pleasant`. Pick an appropriate random
> effects structure and check model assumptions.

> 2.(a) Estimate the time trend of negative affect (`na`) over the study
> period, and (b) whether this time trend differs between mental health
> status groups (`status`). Report standardized effect sizes for the
