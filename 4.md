...
> 1.  Estimate a model that predicts negative affect (`na`) based on
>     whether participants are alone or with others (`soc_alone`) with
>     `use_coffee` and `use_alcohol` as covariates. Pick an appropriate
>     random effects structure based on BIC/LRT and check model
>     assumptions.
> 2.  Estimate the time trend of negative affect (`na`) over the six
>     study days, and whether this time trend differs between mental
>     health status groups (`status`). Report standardized effect sizes.
